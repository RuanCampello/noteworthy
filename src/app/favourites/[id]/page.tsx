...
export default async function Favourite({ params }: { params: { id: string } }) {
  ...
  const favouriteNote: NoteType = favouriteData.notes.find((note: NoteType) => note.uid === params.id)
    ...
    <Resizable>
      <div className='flex h-full'>
        ...
        <div className='w-full px-8 py-6 overflow-y-clip flex flex-col gap-4'>
          <NoteHeader title={title} date={date.seconds} owner={owner} id={uid} />
          <NoteEditor />
        </div>
      </div>
    </Resizable>
